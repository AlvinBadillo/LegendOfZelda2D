...
void DynamicEntity::move(Direction d){          //16 * PIXEL_SCALE;-+
    if (dead)
        return;
    switch (d){
        case UP:
            this->setY(this->getY() - this->speed);
            //this->setY(this->getY() / (16 * handler->PIXEL_SCALE) * (16 * handler->PIXEL_SCALE)); //This truncate to previous 16 per pixel scale
        ...
        case LEFT:
            ...
            //this->setX(this->getX() / (16 * handler->PIXEL_SCALE) * (16 * handler->PIXEL_SCALE));  //This truncate to previous 16 per pixel scale
