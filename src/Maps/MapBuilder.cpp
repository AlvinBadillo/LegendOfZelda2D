...
shared_ptr<Section> MapBuilder::createSection(int startingPixelMapX, int startingPixelMapY, shared_ptr<Link> &linkPtr, ofImage pixelMap) {
    ...
    for (int y = startingPixelMapY; y < startingPixelMapY + VERTICAL_COUNT; y++) {
        ...
        for (int x = startingPixelMapX; x < startingPixelMapX + HORIZONTAL_COUNT; x++) {
            ...
            else if (currentPixel == tilePixelColors.DUNGEON_FLOOR){
                column = 16;
                row = 5;
                collidable = false;
            }
            else if (currentPixel == tilePixelColors.DUNGEON_RED_FLOOR){ //TODO: test to try and make recursive Update: Done
                column = 17;
                row = 7;
                collidable = false;  //When test is done, set false
            }
            else if (currentPixel == tilePixelColors.DUNGEON_OUT){
                column = 14;
                row = 0;
                isADoor = true;
            ...
            //Recursive Dungeon
             else if(currentPixel == tilePixelColors.Move_Up_Block){
                column = 19;
                row = 0;
                collidable = true;
             }
             else if(currentPixel == tilePixelColors.Move_Down_Block){
                column = 20;
                row = 0;
                collidable = true;
            }
            else if(currentPixel == tilePixelColors.Move_Left_Block){
                column = 21;
                row = 0;
                collidable = true;
            }
            else if(currentPixel == tilePixelColors.Move_Right_Block){
                column = 22;
                row = 0;
                collidable = true;
            }
            ...
            // OldMan's Cave Entrance
             ...
             // Dunegon's Entrance
                ...
                row = 1;
                isADoor = true;
                whereTo = MapName::DUNGEON; //need a new map(DONE)(Y)
            ...
            if (collidable) // We found a block that is collidable; add it to the section
            {
                ...
                //TODO: Once the new map is set and RGB values are assigned, uncomment this
                
                else if(currentPixel == tilePixelColors.Move_Down_Block){
                    block = make_shared<MoveDownBlock>(windowXPos, windowYPos, handler);
                }
                else if(currentPixel == tilePixelColors.Move_Up_Block){
                    block = make_shared<MoveUpBlock>(windowXPos, windowYPos, handler);
                }
                else if(currentPixel == tilePixelColors.Move_Right_Block){
                    block = make_shared<MoveRightBlock>(windowXPos, windowYPos, handler);
                }
                else if(currentPixel == tilePixelColors.Move_Left_Block){
                    block = make_shared<MoveLeftBlock>(windowXPos, windowYPos, handler);
                }               
