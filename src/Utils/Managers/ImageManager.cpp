...
shared_ptr<DynamicEntityAnimations> ImageManager::getFrozenTektikeAnimations(){
    DirectionalAnimations movement;
    
    // Tektike's movement animation is the same for all directions
    movement.down = getAnimationFromIndices(87,88);
    movement.right = movement.down;
    movement.left = movement.down;
    movement.up = movement.down;

    frozenTektikeAnimations = make_shared<DynamicEntityAnimations>();

    frozenTektikeAnimations->movement = movement;

    // No attacking animation for Tektike
    frozenTektikeAnimations->attacking = movement;

    frozenTektikeAnimations->damage = getAnimationFromIndices(87, 88);

    return frozenTektikeAnimations;
